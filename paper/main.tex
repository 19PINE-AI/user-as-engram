...
This parametric family has converged on one recipe---a per-user
LoRA~\citep{hu2022lora} on a frozen base's attention projections,
fit on each user's facts (Section~\ref{sec:background})---which we
adopt as our reference \textbf{per-user-LoRA} baseline. A LoRA is
one change layered over the whole model: it keeps a user's facts
and the reasoning skill in the same parameters and, like nudging
every synapse, changes the model everywhere, not just where the
user's facts are relevant.

\paragraph{Why a per-user LoRA's edit is global.}
This is not a tuning artifact---it is what storing a fact
...
We see this as two effects that come apart.
...
\paragraph{The principle: put each fact where storage belongs.}
...
\emph{shared} adapter trained across other users.

\emph{Why it works}: the table is content-addressable and inert off its key,
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=2pt]
  \item \textbf{The cost of storing facts in shared weights}
        (Section~\ref{sec:negative}). A per-user LoRA has no
        address: to fit even one fact, gradient descent must bend
        the same $Q/K/V/O$ directions that are active on unrelated
        text, so the edit changes every forward pass---it
        contaminates globally. An Engram row, by contrast, fires
        only on its trigger N-gram. On a controlled base this is a
        $\sim$34{,}000$\times$ difference in extra loss on unrelated
  ...
  \item \textbf{User as Engram} (Section~\ref{sec:method}):
...
the adapter's inability to provide it. The caution is methodological: judging a per-user
...
\textbf{The mechanism is near-bimodal.} On the balanced 63-pair
corpus (Figure~\ref{fig:multihop}), when the query's suffix
N-gram coincides with the second fact's trigger the gate fires
and recall is high, but when true chaining is required it
collapses
...
condition C in Figure~\ref{fig:layered-conditions}). Can the two be \emph{composed}
...
\begin{itemize}[leftmargin=*,topsep=2pt,itemsep=2pt]
  \item \emph{Multi-fact insertion in the loss.}
        ...
        ceiling. A from-scratch MF pretrain would refine that
  ...
  \item \emph{Random per-batch hash salts.} Salt 50\% of batches
        ...
        tables.
  \item \emph{Recite-then-reason traces.} Mixing ``first surface
        the relevant inserted fact, then reason'' traces into
        Engram pretraining is the most direct attack on the
        shared multi-hop gap.
\end{itemize}
...
not an architectural one, and a full from-scratch MF pretrain is
likely the wrong dial to turn.
...
\begin{figure}[t]
  ...
  \caption{Paraphrase generalisation across 4 facts $\times$ 5 paraphrases
...
\textbf{Dense scaling sharpens generation behaviour.} At the
