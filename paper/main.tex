...
answer LM, User-as-Engram Joint OPT wins under token-F1
(F1 0.233 vs.\ 0.169 MEMMACHINE\_LIKE at d20@1536, full 10
conversations); but \emph{under a Qwen2.5-14B LLM-as-judge,
retrieval baselines instead win by +0.05--0.10 accuracy because
Engram's per-fact OPT only conditions the first answer token}.
Engram's clear win is preserved for paraphrase-style queries
(multi-trigger Engram 96.9\% top-1 vs.\ MEMMACHINE 75.0\%) and
for storage cost;
...
regardless of metric (Section~\ref{sec:density}). (iii) \emph{Joint
OPT improves as we scale Engram's training target}—a
\emph{multi-token answer-conditioned} Engram training objective
would close most of the gap; we did not test this and leave it as
the most direct follow-up.
...
The headline empirical story is split by metric.
On LOCOMO single-hop under token-F1, Joint OPT beats every retrieval
baseline at every dense scale; under semantic LLM-judge, retrieval
baselines (MEM0\_LIKE, MEMMACHINE\_LIKE) instead beat Joint OPT by
0.05--0.10 accuracy, because Engram's per-fact OPT only conditions
the first answer token. The paraphrase win is unambiguous in either
metric: multi-trigger Engram reaches 96.9\% top-1 vs.\ MEMMACHINE's
75.0\% at zero context cost. The clearest contribution is therefore
two-fold: (i) the LOCOMO token-F1 / LLM-judge mismatch points to
training Engram on \emph{multi-token answer-conditioned} targets as
the immediate next step; (ii) the architectural design
(surgical row insertion, additive composition, per-user override
tables, multi-tenant serving) holds regardless of metric and
unlocks zero-leak personal memory at storage costs orders of
magnitude below per-user LoRA. The bottleneck of personal memory
