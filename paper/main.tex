...
and \emph{reasoning skill} (turning facts into answers)---and, as
we argue next, each wants a different kind of memory
(Section~\ref{sec:engram-bio}).
...
\section{Why an engram? Two substrates for memory}
\label{sec:engram-bio}

Underneath the content/skill split is a fact about \emph{where}
each kind of memory wants to live. A transformer's weights are an
excellent substrate for \emph{computation}---they encode smooth,
reusable functions that generalize---but a poor substrate for
\emph{storage}, because they have no addresses. Every weight is
read on every forward pass, so there is no way to write one fact
without also bending the function the model computes for inputs
that have nothing to do with it. A lookup table is the mirror
image: it stores by address---write a slot, read it back
untouched---and stays inert until its key appears, but it
computes nothing on its own. Personal memory needs both at once.
A user's \emph{content} is a storage problem: a pile of specific,
idiosyncratic facts to be written once and read back exactly. The
\emph{reasoning skill} that turns those facts into answers is a
computation problem: a smooth function, shared across users, that
generalizes. The dominant per-user-LoRA recipe forces both into
the same parameters---storage onto the computation
substrate---and pays for it in the damage we measure in
Section~\ref{sec:negative}.

The brain settled this division long ago, and the word
\emph{engram} is a pointer to how. An engram is the physical
trace a memory leaves in neural tissue~\citep{semon1921mneme};
modern neuroscience localizes \emph{engram cells}---sparse neuron
populations whose reactivation triggers recall---largely to the
hippocampus~\citep{tonegawa2015engram}. Complementary learning
systems theory~\citep{mcclelland1995cls,kumaran2016cls} explains
\emph{why} a separate fast store exists at all: a sparse,
pattern-separated hippocampal code can write a new episode
\emph{without} catastrophically overwriting the slow, distributed
cortical representations that hold general skills---exactly the
interference a per-user LoRA suffers and the hippocampus avoids.
User as Engram makes that division architectural
(Table~\ref{tab:bio-mapping}): each user's facts are the fast,
local trace, written into a hash-addressed table the model was
pretrained to read from, while the shared adapter and frozen
backbone are the slow, distributed skill. Because the table is
content-addressable and its read is gated on the trigger N-gram,
a written fact perturbs the model only where that fact is
addressed---\emph{exactly} zero elsewhere, as we verify in
Section~\ref{sec:locality}---which is the engineering restatement
of the interference-avoidance the hippocampus exists to provide.
...
A per-user LoRA edits $Q/K/V/O$ in every layer, so its edit is
unconditioned---part of every forward pass. This section takes
that fact apart into the two things it actually does, which
behave very differently. The \emph{architectural} cost---loss on
text unrelated to the user---is the direct signature of storing a
fact in a shared function: large, and the same in kind on every
base. The \emph{user-visible} cost---degraded reasoning---is
downstream of the first but mediated by how robust the base is,
and so it varies. Reading the two together explains both why the
literature disagrees about whether per-user LoRA ``works'' and
why no training recipe repairs it.
...
The split is mechanistically clean. The bpb damage above is the
same in kind on every base, because it is a property of the
parameterization; what differs is whether the model's
\emph{readout} survives it. An instruction-tuned model has
redundant, well-conditioned reasoning circuits that tolerate a
small global nudge, so its answers barely move; a base LM's
completion behaviour is fragile, so the same nudge derails it
(here, 17/20 users worse). The lesson is methodological:
evaluating per-user LoRA only by task accuracy on a robust base
hides the edit, because robustness masks a perturbation that is
nonetheless always present. The early $n{=}10$ pilot's ``5/10
worse'' was this fragile-base signal read at small $n$; at scale
the user-visible effect shrinks toward zero on instruction-tuned
bases even as the architectural damage stays put.

\subsection{Why no recipe fixes this}

If contamination is what storing a fact in a shared function
costs, then no way of \emph{training} that function should remove
it---only changing where the fact lives can. The two recipe-level
fixes we tried bear this out. \emph{Recite-then-reason} traces
teach the adapter to surface a fact before using it: this closes
the reasoning gap \emph{within} the training schema but collapses
when the test fact comes from a new one, because it sharpened the
skill without making the storage local. A full-parameter
\emph{meta-train} over the adapter distribution lifts reasoning
on training users, transfers essentially nothing to held-out
ones, and misaligns the frozen adapters enough to break direct
recall (Appendix~\ref{app:userlora}). Both attack the symptom
(reasoning) and leave the cause (storage on a computation
substrate) untouched. The fix is to move the fact, not retrain
the delta: Section~\ref{sec:method} puts content on a storage
substrate, and Section~\ref{sec:layered} keeps the reasoning
skill on the shared computation substrate where it belongs.
