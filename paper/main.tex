...
and \emph{reasoning skill} (turning facts into
answers).
...
A per-user LoRA adds a small change to $Q/K/V/O$ in
\emph{every} transformer layer; that change is part of every
forward pass, including ones whose tokens have nothing to do with
the user's facts. This shows up as two separate effects when we
measure. The damage to the model itself---extra loss on text that
has nothing to do with the user---is large, and it appears on
every base model we tried. The effect a user would actually
notice in the reasoning is different: instruction-tuned models
shrug off the disturbance, plain base models do not. The claim we
lean on is the first one (Section~\ref{sec:negative}).

\paragraph{The fix: a local trace plus a shared skill.}
An edit avoids this damage only if it activates when the user's
fact is relevant and does nothing at all everywhere else---the
same trick the hippocampus gets from sparse coding. We propose
\textbf{User as Engram}: store each of
Maya's facts as a surgical write to the hash-keyed embedding
table of an Engram-pretrained model~\citep{cheng2026engram},
whose gated lookups fire only on suffix N-grams via
deterministic multiplicative-XOR hashing. Per user, we write one
fact row per trigger N-gram into an \emph{override map} applied
per request; the reasoning skill that interprets those facts
lives in one \emph{shared} adapter trained across other users.
This split between content and reasoning skill is the artificial
counterpart of
complementary learning systems~\citep{mcclelland1995cls}
(Section~\ref{sec:engram-bio}). We can measure this locality
exactly, not just approximate it in the limit: on Mini-Engram-d12,
a UNEMBED\_P insertion at the last Engram layer changes the
...
before~\citep{mccloskey1989catastrophic}. Decades of
continual-learning work fight this by protecting important
weights~\citep{kirkpatrick2017ewc} or replaying old data, and the
problem persists for large language
models~\citep{shi2024continual}. Complementary learning systems
theory~\citep{mcclelland1995cls,kumaran2016cls} argues the brain
avoids it with a fast, sparse hippocampal store sitting beside a
slow cortex---the separation we make architectural. Where
continual learning asks one set of weights to absorb new facts
gracefully, we sidestep the interference entirely by writing each
fact to its own address and leaving the shared weights untouched.

\section{Per-user LoRA contaminates globally}
\label{sec:negative}

A per-user LoRA edits $Q/K/V/O$ projections in every transformer
layer; the edit is global by design. We measure its two
...
unchanged to four decimal places in every seed. The reason is
plain: \textbf{LoRA's edit is global;
...
We replicate the per-user LoRA recipe (rank-64 NTP on
...
$+0.217$, worse-fraction 0--20\%. The original LoRA pilot's
...
Section~\ref{sec:layered} restores the reasoning skill that per-user
...
\subsection{Why ``Engram''? A biological grounding}
\label{sec:engram-bio}

The name is not incidental. An \emph{engram} is the physical
trace a memory leaves in neural tissue~\citep{semon1921mneme},
and modern neuroscience localises \emph{engram cells}---sparse
neuron populations whose reactivation triggers recall---largely
to the hippocampus~\citep{tonegawa2015engram}. The
complementary-learning-systems account~\citep{mcclelland1995cls,
kumaran2016cls} explains why the brain bothers with a separate
fast store: a sparse, pattern-separated hippocampal code can
write a new episode \emph{without} catastrophically interfering
with the slow, distributed cortical representations that encode
general skills. User as Engram is the same division of labour in
a transformer (Table~\ref{tab:bio-mapping}): per-user rows are
the fast, local trace; the shared adapter and frozen backbone
are the slow, shared skill; and our measured locality
($\Delta$bpb $+0.00005$ on unrelated text,
Section~\ref{sec:locality}) is the engineering restatement of
the very interference-avoidance property that the hippocampus
exists to provide.

\begin{table}[t]
  \centering
  \caption{User as Engram as an artificial complementary learning
  system. The architecture mirrors the brain's separation of a
  fast, sparse, local episodic store from a slow, distributed
  skill store; the right column is realized, not metaphorical.
  We claim a \emph{computational} analogy---Engram rows are
  written by gradient optimisation, not Hebbian plasticity, and
  the lookup is feed-forward, not a recurrent attractor---so the
  mapping is a design principle, not a claim of biological
  fidelity.}
  \label{tab:bio-mapping}
  \resizebox{\linewidth}{!}{
  \begin{tabular}{ll}
    \toprule
    Complementary learning systems (brain) & User as Engram (this work) \\
    \midrule
    Hippocampal engram: sparse, local episodic trace & Per-user Engram row: sparse hash-keyed edit (88\,KB) \\
    Neocortex: slow, distributed general skills & Shared LoRA + frozen backbone: shared reasoning skill \\
    Pattern separation avoids catastrophic interference & Locality: $\Delta$bpb $+0.00005$ on unrelated text \\
    CA3 pattern completion: partial cue $\to$ full memory & Gated lookup: trigger N-gram $\to$ stored value \\
    Systems consolidation: hippocampus $\to$ cortex & Folding recurring content into the shared skill \\
    \bottomrule
  \end{tabular}}
\end{table}
...
isolated question is which method extracts the answer most
...
10-conversation LOCOMO setup (Figure~\ref{fig:locomo-judge}):
...
\textbf{Under the semantic LLM judge, first-token Engram trails
retrieval on LOCOMO single-hop, and multi-token OPT reverses
it.} With first-token OPT, token-F1 over-credited Engram by
0.05--0.10 because it rewards the correct first answer token
without penalising a noisy continuation
(Figure~\ref{fig:metric-mismatch}: Engram points sit above the
$y{=}x$ line, retrieval points sit on it). Engram still wins paraphrase
queries (96.9\% top-1 vs.\ MEMMACHINE's 75.0\%,
Figure~\ref{fig:memsystems}; unchanged by the judge
correction since paraphrase scores single-token recall) and
storage (88\,KB / 100 facts, metric-independent).

\textbf{Multi-token OPT closes the judge gap.} The gap is driven
by per-fact OPT training only the first answer token. Replacing
$-\log p(y_0 \mid \text{trigger})$ with the autoregressive sum
$-\sum_t \log p(y_t \mid \text{trigger}, y_{<t})$ pushes the
residual stream toward the \emph{full} answer
(Figure~\ref{fig:locomo-judge}): at d20 under matched
multi-token pipelines, J-OPT-multi reaches token-F1 0.310 vs.\
0.197 for the best retrieval baseline ($+57$\%) and LLM-judge
0.225 vs.\ 0.174 ($+29$\%), with the cross-over to
Engram-winning between d12 v2 and d12@1280.
...
compose across triggers by design. The recite-then-reason
recipe (Section~\ref{sec:discussion}) is the natural mitigation;
multi-fact-in-the-loss pretraining (Section~\ref{sec:future}) is
the structural fix. (An earlier overlap-heavy 8-pair pilot read
75\%; the balanced 63-pair corpus in
Figure~\ref{fig:multihop} corrects it.)
...
serving protocol is \emph{zero by design} regardless of
pool size.
...
\section{Layered architecture: content + reasoning skill}
\label{sec:layered}
...
\begin{itemize}[leftmargin=*,topsep=2pt,itemsep=1pt]
  ...
  \item \textbf{Oracle retrieval does not help.} G$'$ uses the
        ...
        indirect reasoning without the shared skill.
...
realistic ``user-as-RAG'' baseline, yet F matches it without any
retrieval and J overtakes it once context is free---the clean
trade-off of Figure~\ref{fig:pareto-rag}.
...
\textbf{Which method wins shifts with deployment scale.} At
...
cleanly once the inference budget is unpinned
(Figure~\ref{fig:mf-finetune}). With 12k Joint OPT steps,
...
cases matter for different deployment shapes:
\begin{itemize}[leftmargin=*,topsep=2pt,itemsep=1pt]
...
(Section~\ref{sec:density}). The ceiling at fixed
OPT-steps is real and holds at every dense size; but the ceiling
at unlimited OPT-steps is meaningfully higher ($+6.6$\,pp on
baseline d12@1280 at $n{=}1000$ when steps go
$8\text{k} \to 12\text{k}$). The economic question---what does
each registration second cost?---makes MF a clear win for
production deployments; the architectural
...
them.} The usual approach asks a single small per-user adapter to
do both at once, and that is the root of the trouble this paper
untangles. An adapter that has to \emph{hold} the facts cannot
avoid also reshaping how the model thinks: each fact it learns
nudges weights the model uses everywhere, so remembering and
reasoning end up pulling on the same knobs, and the user pays for
it in damaged general knowledge and weaker reasoning. The fix is
not a cleverer training run but a different place to put each
piece---keep a user's facts as a small, local change in one spot,
and keep the reasoning skill in a single model that everyone
shares.

Three plain lessons carry the result. \emph{First, where you
write a memory decides whether it stays put.} Storing a fact in
one slot of a lookup table changes that slot and nothing else;
fine-tuning an adapter smears the same fact across the whole
model. That gap is built into the method, not something you tune
away---it holds on every model we tried, even when a particular
benchmark happens not to show it. \emph{Second, being able to
recall a fact is not the same as being able to reason with it.}
The mechanism that makes our writes precise works by matching the
words in a question, not their meaning, so it can look a fact up,
and can even look like it is combining two facts when the wording
lines up---but it cannot actually put them together. The
reasoning has to come from somewhere else: the shared skill. That
is why pairing the two beats every method that tries to do
everything in one place, and why the pairing keeps working even
when we hand users a kind of question that breaks a
recite-then-reason recipe. \emph{Third, which method wins depends
on how much you are actually serving, not on how large a
benchmark is.} A private per-user store whose size and speed do
not change as a user piles up facts will overtake a retrieval
system whose accuracy slips as its pile of documents grows; the
real question is never ``which is better'' in the abstract but
``which is better for the number of users, and the facts per
user, you actually have.''

The recommendation falls out of these lessons: keep each user's
facts in a small private edit, put the shared reasoning in one
common model, and leave open-ended lookups to retrieval. The
practical wins then come on their own---no user can ever read
another's memory, and total storage grows with the number of
users instead of the size of the model. Two things are still
open. The sharpest, and the most interesting, is that our memory
can match its stored facts but cannot chain them: a version that
could combine its own memories, rather than only look them up,
would close the last case where plain retrieval still wins on
meaning. And the shared skill is trained once and then frozen;
teaching it to keep learning as people's questions change over
...
\section{Appendix: LOCOMO full results}
\label{app:locomo}
...
{\scriptsize
\begin{verbatim}
git clone https://github.com/19PINE-AI/user-as-engram
...
\section{Appendix: Extended experiments and ablations}
\label{app:extended}
...
(i) Cross-schema generalisation was the most important open
...
Section~\ref{sec:layered-sft} shows F still beats B on every
measure on a Q/A-format-adapted Mini-Engram-d20 ($2.8\times$ on
