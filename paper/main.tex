...
position}, through every subsequent layer. We present
(i) a nano-scale public reproduction of Engram at four dense sizes
spanning 137\,M to 1.22\,B parameters—the first publicly available
Engram models outside DeepSeek-AI;
(ii) a $5 \times 3 \times 2$ \textbf{capacity ablation} across
Engram-table size, token budget, and dense scale, showing the optimal
Engram capacity is invariant across dense sizes and only the token
budget needs to scale;
(iii) \textbf{Joint OPT}, a sparse-row joint training method that
recovers recall under high fact density (68\% top-1 / 96\% top-5 at
100 facts/user vs.\ 36\% / 54\% for independent OPT);
(iv) \textbf{fact-count scaling to 1\,000 facts}, where per-fact
independent OPT recall is flat (d12@1280 holds 0.98 USER OPT top-1
from $n{=}100$ to $n{=}1000$);
(v) on the \textbf{LOCOMO} single-hop memory benchmark with a fixed
answer LM, User-as-Engram Joint OPT achieves \textbf{F1 0.219 at
d20@1536 vs.\ 0.140 for the strongest retrieval baseline}
(+57\%), and the lead widens monotonically with dense scale;
(vi) a working multi-tenant serving system with sub-millisecond
override swap, 47.9 req/s throughput, and zero cross-user leak by
construction;
(vii) a comprehensive comparison vs.\ ICL, RAG, SFT-LoRA, and POLAR,
where Joint OPT uses 88\,KB of storage—153$\times$ less than the
POLAR-class LoRA at the same fact density.
Code, models, and the full evaluation suite are released.
\end{abstract}
...
\begin{figure}[t]
  ...
  135$\times$ smaller per-fact storage. Retrieval baselines sit
  on the steep low-storage side of the curve but cap out below
  Engram on quality.}
  \label{fig:pareto}
\end{figure}
...
\begin{figure}[t]
  ...
  (50\,K $\times$ 256, 51\,M Engram params). Top-5 improves
...
\begin{table}[h]
  ...
  scales at fixed Engram capacity (51\,M params).}
...
retrieval baselines hit 1.00 on trigger-exact at every scale;
User-as-Engram multi-trigger holds 0.94–0.97 paraphrase top-1
versus retrieval baselines at 0.78–0.83. The verdict—\emph{retrieval
wins when the query is verbatim the stored prefix, multi-trigger
Engram wins under paraphrase}—does not change with dense scale.
Engram numbers are essentially fixed; only the retrieval-baseline
numbers move slightly because their answering-LM quality
improves.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.78\linewidth]{figs/fig_locomo_scaling.pdf}
  \caption{LOCOMO single-hop token-F1 across four Mini-Engram
  sizes, all at Karpathy 12 t/p of scaling-params. User-as-Engram
  Joint OPT (blue) scales monotonically from 0.161 (137\,M) to
  0.219 (1.22\,B); retrieval baselines climb more slowly (the
  answering-LM quality improves but the substrate doesn't). The
  Engram lead over MEMMACHINE\_LIKE is +115\% at the smallest
  dense and +57\% at the largest.}
...
\begin{figure}[t]
  ...
  Left: LOCOMO Joint OPT F1 climbs from 0.161 (best d8) to 0.219
  (d20@1536); MEMMACHINE\_LIKE retrieval baseline climbs more
  slowly because its quality is bounded by sentence-encoder
  retrieval, not the LM. Right: val\_bpb (grey) drops monotonically
  with dense+tokens, while E1 USER OPT (blue) saturates at 1.00
  from d12@768 onward (small dip at d20 at iso-12 t/p).}
