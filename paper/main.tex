...
\section{Two kinds of memory: content and skill}
\label{sec:engram-bio}

Personal memory has two parts that pull in different directions.
A user's \emph{content}---Maya's cardiologist, her penicillin
allergy---is a set of specific, idiosyncratic facts that should
be written once, read back exactly, and that differ from user to
user. The \emph{reasoning skill} that turns those facts into
answers (\emph{``where should I get a heart check-up near my
daughter?''}) is the opposite: one competence, shared across
users, that should generalize. The two want different homes:
content wants a private, addressable store that can be written
and composed cheaply per user, while skill wants the shared,
generalizing weights a single model already supplies. The
dominant per-user-LoRA recipe puts both into one per-user weight
edit, and Section~\ref{sec:negative} shows why that is the wrong
home for the content.

The brain draws the same line, and the word \emph{engram} points
to it. An engram is the physical trace a memory leaves in neural
tissue~\citep{semon1921mneme}; modern neuroscience localizes
\emph{engram cells}---sparse neuron populations whose
reactivation triggers recall---largely to the
...
why a separate fast store exists at all: a sparse,
pattern-separated hippocampal code can write a new episode
without overwriting the slow, distributed cortical
representations that hold general skills. User as Engram makes
that division architectural (Table~\ref{tab:bio-mapping}): each
user's facts are the fast, local trace, written into a
hash-addressed table the model was pretrained to read from, while
the shared adapter and frozen backbone are the slow, distributed
skill. The mechanism that keeps the two from interfering---the
table's content-addressable, gated read---is described in
Section~\ref{sec:method} and verified exact in
...
We instantiate the ``local per-user edit'' of
Section~\ref{sec:engram-bio} concretely: surgically write
...
This is precisely the addressable, sparse storage substrate that
Section~\ref{sec:engram-bio} argued personal facts want.
