...
adapter, an artificial complementary learning system. This layered
...
Against retrieval, a per-user Engram table does not grow with the
population the retriever must search, so past $\sim$100 facts it overtakes a retrieval pipeline on a
$2.5\times$ larger model. Every method that reaches near-perfect
recall pays for it in weaker reasoning; they differ only in
\emph{where} that price is paid: the shared weights (LoRA), one
address (Engram), or a search index (RAG).
\end{abstract}
...
\begin{figure}[t]
  ...
  pretraining (grey). A write touches only that user's rows
...
LoRA~\citep{hu2022lora} fit on each user's facts
(Section~\ref{sec:background}); we adopt it as our reference
\textbf{per-user-LoRA} baseline because it is the in-weights method
closest to ours, the sharpest test of \emph{where} a fact should live
once you store it parametrically at all. A LoRA has no address: to
...
side-effect of training. This costs extra loss on \emph{every} base we
tried; the damage a user actually notices, worse reasoning, appears
only on a weak base, because a strong instruction-tuned base answers
the question on its own and hides it (Section~\ref{sec:negative}).

\paragraph{The fix: content in a memory table, skill in a shared adapter.}
...
table is content-addressable and inert off its key, so writing a fact
changes the output only where its trigger N-gram is read: \emph{exactly}
0.000 everywhere else, through every later layer, not merely small
...
writes are isolated (facts compose and never leak across tenants), the
...
The three families are not really competing on quality: each reaches
near-perfect direct recall and each pays for it in weaker reasoning
over the stored facts. What differs is \emph{where} the price lands and
how fast it grows: a per-user LoRA pays in the shared weights, on
every query and stacking across users; an Engram row pays at the one
address its fact occupies, growing only with a single user's facts;
retrieval pays in a search index that degrades with the pool size.
