...
the trigger N-gram's hash address. The edit is \emph{local}: on
Mini-Engram-d12 we measure that insertion changes the residual
stream by L2 norm 123.0 at the trigger position in the first
post-Engram layer (decaying to 1.54 by the final layer) and
\emph{exactly 0.000 at every other position}, through every
subsequent attention/MLP layer. We present
(i) a nano-scale public reproduction of Engram at four dense sizes
spanning 178\,M to 1.22\,B total parameters (137\,M to 1.17\,B
dense; $\sim$53\,M Engram-table params)—the first publicly available
...
answer LM, multi-token Joint OPT wins under both metrics at
d20@1536: token-F1 0.310 vs.\ 0.197 best retrieval
(+58\%), LLM-judge accuracy 0.225 vs.\ 0.190 best retrieval
(+18\%, both pipelines). The na\"ive first-token OPT objective
wins on token-F1 but loses under LLM-judge by $-$0.05 (the first
generated token is the gold token but the 15-token continuation
is unguided); \textbf{multi-token answer-conditioned OPT closes
the gap and flips the ranking back to Engram winning}. Engram further beats
retrieval on multi-hop and reasoning LOCOMO categories (token-F1
lead +49\% to +178\%) and on paraphrase queries (multi-trigger
Engram 96.9\% top-1 vs.\ MEMMACHINE 75.0\%); retrieval still wins
on LOCOMO open-domain ($-$53\% Engram), where the answer is
verbatim in a single evidence sentence;
(vi) a working multi-tenant serving system with sub-millisecond
override swap, 232\,req/s throughput on a single GPU at d12@1280
scale, and zero cross-user leak by construction;
(vii) a comprehensive comparison vs.\ ICL, RAG, SFT-LoRA, and POLAR,
where Joint OPT uses 88\,KB of storage—161$\times$ less than the
POLAR-class LoRA at the same 100-fact density (14.2\,MB
rank-64 fp32).
...
insertion at the last Engram layer changes the residual stream by
L2 norm 123.0 at the trigger position (decaying through subsequent
layers to 1.54 at the final layer) and \emph{0.000 at every other
position}, in every layer after the insertion.
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  ...
  \item \textbf{Nano-scale public reproduction of Engram} (Section~\ref{sec:repro})
        at four dense sizes spanning 178\,M to 1.22\,B total parameters
        (137\,M to 1.17\,B dense),
  ...
  \item \textbf{User-as-Engram method} (Section~\ref{sec:method}) with
        ...
        training). At 100 facts/user, Joint OPT roughly doubles
        both top-1 (68\% vs.\ 36\%) and top-5 (96\% vs.\ 54\%)
        vs.\ independent OPT, at the same storage.
  ...
  \item \textbf{Dense-size scaling}
        ...
        first-token token-F1 scales monotonically with dense size
        from 0.134 (178\,M) to 0.233 (1.22\,B). \emph{Under
        LLM-as-judge with the first-token objective}, the ranking
        reverses and retrieval baselines beat Joint OPT by
        +0.05--0.10 accuracy because Engram's OPT only conditions
        the first answer token. Multi-token OPT
        (Table~\ref{tab:locomo-multitoken}) flips the ranking back:
        at d20@1536 it reaches 0.310 token-F1 and 0.225 LLM-judge
        accuracy, beating both MEM0 and MEMMACHINE. The paraphrase
        win at 96.9\% top-1 is metric-independent.
...
\paragraph{Cheng et al.\ \S6.3 sensitivity asymmetry reproduced.}
...
\paragraph{Cheng et al.\ \S6.1 effective deepening reproduced.}
...
\begin{table}[h]
  ...
  on ClimbMix-400B, bf16, single Blackwell GPU. ``Total'' includes
  the $\sim$53\,M Engram-table params; ``scaling'' is the dense
  transformer body (the figure that drives the 12 t/p schedule).
  LOCOMO J is Joint OPT first-token token-F1 on the full
  10-conversation single-hop set, matching
...
many users have the same favourite spice drawn from a 20-item pool,
...
625\,M scale; the paper's 27\,B headline number requires the paper's
compute budget).
...
User-as-Engram edits are \emph{local} and reasoning-positive
where it matters (multi-hop +178\% and reasoning +58\% token-F1
on LOCOMO). The local-edit property—we measure exactly 0.000
contamination of non-trigger forwards through 5 attention layers—is
the architectural reason User-as-Engram doesn't pay the reasoning
tax that LoRA-as-memory does. With Joint OPT, per-user override
tables, and additive multi-domain composition, User-as-Engram
serves millions of users at $\sim$100\,GB storage and 232\,req/s
on a single GPU at d12@1280 scale, with zero cross-user leak by
construction. Our four-size scaling study trains Mini-Engrams from
178\,M to 1.22\,B total parameters on a single Blackwell GPU. The
30-cell capacity ablation pins the optimal Engram capacity at
\texttt{large} (50\,K $\times$ 256, $\sim$53\,M params) across both
probed dense sizes; per-fact independent OPT recall is flat up to
1\,000 facts.
...
(d20@1536: token-F1 0.310 vs.\ best retrieval 0.197 (+58\%);
LLM-judge 0.225 vs.\ 0.190 (+18\%, best retrieval across
pipelines)). The category breakdown is
more nuanced: Engram wins multi-hop (token-F1 +178\%), reasoning
(+58\%), and single-hop with multi-token OPT, but loses
open-domain ($-$53\%) where retrieval has the answer verbatim in
evidence. The clearest practical contribution is therefore
twofold: (i) Engram's surgical training objective should match
...
memory at storage costs orders of magnitude below per-user LoRA.
The bottleneck of personal memory moves from gradient training to
substrate selection—and the right substrate is hash-keyed embedding
tables.
