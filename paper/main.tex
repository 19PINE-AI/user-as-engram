...
mathematically untouched---a roughly $33{,}000\times$ smaller
footprint on unrelated text (mean over three seeds), by design.
...
$5.6\times$ higher indirect-reasoning accuracy on average (up to
$7.4\times$), and never makes a single user worse at reasoning
than the untouched base. We can also open the edit
up and watch it work on the trained model. Writing a fact switches
on the memory lookup at exactly its trigger and adds the value that
drives the answer---leaving every other position
unchanged down to the last bit---and it stops working if written
into the wrong layer. Each step is measured, not assumed. Against
retrieval, which method wins is decided by deployment, not benchmark
size: a per-user Engram table does not grow with
...
The right place for a fact is a memory table, not the shared
weights.
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=2pt]
  \item \textbf{The cost of storing facts in shared weights---and
        ...
        $\sim$33{,}000$\times$ difference in extra loss on unrelated
        text (3-seed mean)---a property of how a LoRA stores a fact,
        not of how we tuned it. This extra loss is always there; what a user
...
\begin{figure}[t]
  ...
  contamination is paid once and amortized across all users.
...
We compare six parametric conditions, which we refer to by both a
shown as the six bars (in this order) in
...
\begin{figure}[t]
  ...
  latency, a sub-millisecond override apply (full per-component
  breakdown in Appendix~\ref{app:cdf}), 88\,KB/user, and 62\%/96\%
...
Choosing a personal-memory method is therefore not about which one
avoids the price---none does---but about relocating it to the place
where, for a given deployment, it is cheapest.
