...
Qwen2.5-3B base, 10 synthetic users $\times$ 50 facts) and find that
\emph{the adapter actively makes reasoning worse}:

\begin{itemize}[leftmargin=*,topsep=2pt,itemsep=1pt]
  \item Direct recall with adapter: \textbf{1.000} (perfect
        memorisation).
  \item Indirect recall with adapter: \textbf{0.150}—\emph{below}
        the no-adapter base of \textbf{0.170}.
  \item In \textbf{5 of 10 users}, the adapter is strictly worse
        than the base on indirect questions.
\end{itemize}

The adapter is not amnesiac—CoT-prompted recall reaches the
ICL-with-CoT ceiling at 0.30, so the facts are encoded—but
\emph{using} the adapter at inference contaminates the model's
general reasoning whenever the question's surface form drifts from
the trained QA pattern. The mechanism is structural: a LoRA delta
is a \emph{global} edit on $Q/K/V/O$ in every layer, so every
forward pass—relevant or not—sees the perturbation.
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  \item \textbf{A reasoning-negative finding with a mechanistic
        diagnosis} (Section~\ref{sec:negative}): per-user LoRA is
        strictly worse than the base in 5/10 users on indirect
        recall, despite saturating direct recall—because the edit
        is global.
...
\paragraph{Take-away.} \emph{The architecture of memory determines
whether memory helps or hurts reasoning.} Global edits like LoRA
purchase recall at the cost of cross-question interference; local
edits like Engram-row insertion buy zero interference and—with the
right training objective—also win on the categories where memory
should help (chain-of-fact recall). They lose where memory can't
help (free-form synthesis from never-seen evidence). That is the
substrate-selection question we make precise.
...
\begin{table}[t]
  \centering
  \caption{Layered architecture: 6 conditions $\times$ 20 test users on
  Mini-Engram-d20 ($n{=}20$). All training is independent (per-user LoRA,
  per-user Engram, and shared LoRA each trained alone). ``worse'' = fraction
  of users with indirect\_any strictly below the no-edit baseline.}
  \label{tab:layered-headline}
  \resizebox{\linewidth}{!}{
  \begin{tabular}{lccccccc}
    \toprule
     & per-user & shared & direct & direct & indirect & indirect & $\Delta$bpb \\
    code & LoRA r=64 & LoRA r=16 & top-1 & top-5 & top-1 & any & (worse/20) \\
     & + Engram J-OPT? & + Engram J-OPT? & & & & & \\
    \midrule
    A & none & none & 29\% & 38\% & 14\% & 19\% & 0.000 (0/20) \\
    B & LoRA only & no & 100\% & 100\% & 36\% & \textbf{7\%} $\downarrow\downarrow$ & \textbf{+1.559 (17/20)} \\
    C & Engram only & no & 100\% & 100\% & 14\% & 23\% & +0.0001 (0/20) \\
    D & LoRA + Engram & no & 100\% & 100\% & 36\% & \textbf{8\%} $\downarrow\downarrow$ & +1.419 (16/20) \\
    E & none & LoRA only & 54\% & 76\% & 62\% & 44\% $\uparrow\uparrow$ & +0.386 (0/20) \\
    \textbf{F} & \textbf{Engram} & \textbf{LoRA} & \textbf{100\%} & \textbf{100\%} & \textbf{61\%} & \textbf{44\%} & \textbf{+0.386 (0/20)} \\
    \bottomrule
  \end{tabular}}
\end{table}
...
of two axes—\emph{how much per-user storage is acceptable}, and
\emph{whether the question is a chain-of-fact recall or
free-form synthesis}. The cells we recommend:
\begin{itemize}[leftmargin=*,topsep=2pt,itemsep=1pt]
  \item \emph{$<$10 facts/user, conversational AI.} ICL or RAG
        with a strong retriever: lowest storage, highest recall,
        but cannot internalise reasoning steps.
  \item \emph{10--500 facts/user, millions of users, storage is
        the binding constraint, 65--98\% multi-fact top-1/top-5
        acceptable.} User-as-Engram + Joint OPT. Storage scales
        linearly at $\sim$1\,KB/fact, and the multi-hop and
        paraphrase wins
        (Sections~\ref{sec:multihop},~\ref{sec:paraphrase}) accrue
        here.
  \item \emph{Enterprise personalisation requiring 100\% top-1,
        $<$10K users, can afford 14\,MB/user.} Per-user LoRA with
        S-LoRA serving—and accept the indirect-recall tax.
  \item \emph{Long-form documents, citations, provenance,
        open-domain synthesis.} RAG: retrieval wins LOCOMO
        open-domain by 53\%
        (Table~\ref{tab:locomo-categories}).
\end{itemize}
The crucial line is the third: \emph{any} substrate that hits
100\% direct top-1 on personal facts will incur some indirect-
reasoning tax; the question is whether the tax is structural
(global LoRA) or constructive (local Engram, where the trigger
N-gram is the contract).
...
across users; open-domain synthesis $\to$ RAG. \textbf{(ii)}
\emph{Match the training objective to the deployment metric}—
multi-token answer-conditioned OPT where free-form answers matter,
first-token OPT where the inserted span suffices. \textbf{(iii)}
\emph{Treat substrate selection, not gradient training, as the
bottleneck of personal memory}—gate-vs-store separation, layered
composition, per-user override tables, and multi-tenant serving are
architectural decisions, and together they unlock zero-leak personal
memory at $\sim$100\,GB total storage for 1\,M users and 232\,req/s
on one GPU.
