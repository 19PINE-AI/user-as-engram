...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  ...
  \item \textbf{Nano-scale public reproduction of Engram} (Section~\ref{sec:repro})
        at 137M and 339M parameters in $\sim$50 min and $\sim$3 h on a
        single GPU. To our knowledge the first publicly available
        Engram model outside DeepSeek-AI.
...
+~AdamW~\citep{jordan2024muon}) and train two models on a single
GPU: \emph{Mini-Engram-d8} (137M params, 524M ClimbMix tokens,
$\sim$50 min) and \emph{Mini-Engram-d12} (339M params, 786M tokens,
$\sim$3 h).

\begin{table}[h]
  \centering
  \caption{Mini-Engram pretraining.}
  \label{tab:miniengram}
  \begin{tabular}{lccc}
    \toprule
     & base d8 & engram d8 & engram d12 \\
    \midrule
    depth $\times$ width & 8 $\times$ 512 & 8 $\times$ 512 & 12 $\times$ 768 \\
    Engram layers & --- & 2, 5 & 2, 7 \\
    Engram slots & --- & 644{,}798 & 1{,}605{,}126 \\
    Engram params & --- & 10.8 M & 52.9 M \\
    total params & 125.8 M & 136.7 M & 339.2 M \\
    ClimbMix tokens & 524 M & 524 M & 786 M \\
    final val\_bpb & 0.929 & 0.924 & \textbf{0.849} \\
    \bottomrule
  \end{tabular}
\end{table}
...
question with the judging-LM question. (iii) The Mini-Engram-d12
floor is low; absolute numbers will rise with our planned
600M and 1B Mini-Engrams (Section~\ref{sec:future}). (iv) We
...
\begin{table}[h]
  ...
  pairs, Mini-Engram-d12).}
  \label{tab:multihop}
  \begin{tabular}{lc}
    \toprule
    metric & value \\
    \midrule
    per-fact direct recall (sanity) & 15/16 = 93.8\% \\
    multi-hop top-1 & 6/8 = 75.0\% \\
    multi-hop top-5 & 6/8 = 75.0\% \\
    \bottomrule
  \end{tabular}
\end{table}
...
Our method assumes an Engram-pretrained base. We trained two
ourselves; production deployment depends on either DeepSeek
releasing weights or pretraining your own ($\sim$3 h on a single GPU
at our d12 scale; the paper's 27B headline number requires the paper's
compute budget).
...
\subsection{Future work: scaling and recipe changes}
\label{sec:future}

Our 339M Mini-Engram-d12 is the smallest scale at which we observe
clean architectural signal; the paper's 27B headline is 80$\times$
larger and trained on 333$\times$ more tokens. Two complementary
directions:

\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=2pt]
  \item \textbf{Scale in parameters.} We are training two larger
        Mini-Engrams with the same recipe: a 625M model
        (depth 12, $d_\text{model}=1280$, 20 heads) and a 1.22B model
        (depth 20, $d_\text{model}=1536$, 24 heads), both with the
        same Engram configuration as d12 (50K hash slots per
        N-gram, 8 heads, 256 embed-per-N-gram, layers 2 and
        7/11). Each consumes $\sim$786M ClimbMix tokens
        (6000 iterations $\times$ 131K-token batches), so they
        remain Chinchilla-undertrained relative to the paper but
        sit at single-GPU budgets ($\sim$3 h and $\sim$8 h
        respectively on one Blackwell). The expected payoff is
        partly absolute LOCOMO numbers, partly tightening the
        reproduction signal in
        Section~\ref{sec:repro}---our suppression $\Delta$bpb of
        $+0.035$ is $\sim$10$\times$ smaller than the paper's, and
        scale is the most likely cause.
  \item \textbf{Scale in training recipe, not just parameters.}
        Three recipe changes target the limitations we document
        and matter more than raw scale:
        \begin{itemize}[topsep=2pt,itemsep=1pt]
          \item \emph{Multi-fact insertion in the loss.} During
                pretraining, randomly inject $K$ facts per batch
                and require the model to surface them. This
                trains the gate to handle multi-fact density
                natively, addressing the within-user density
                ceiling visible in
                Table~\ref{tab:joint-opt-density}.
          \item \emph{Random per-batch hash salts.} Salt 50\% of
                batches with a random per-batch user salt during
                pretraining. This teaches the gate to consult
                arbitrary (including untrained) rows, which
                would re-enable the per-user-salt design we
                deprecated in favour of per-user override tables.
          \item \emph{Recite-then-reason traces.} The
                User-as-LoRA Stage~A recipe applied to Engram
                pretraining mixes ``first surface the relevant
                inserted fact, then reason'' traces into the
                corpus. This is the most direct attack on the
                shared multi-hop gap.
        \end{itemize}
\end{enumerate}

None of (i)--(ii) requires more raw compute than the 1B run we
are launching. We expect the recipe changes (ii) to deliver
larger gains per FLOP than the parameter scaling (i) on the
...
zero cross-user leak by construction. The bottleneck of personal
