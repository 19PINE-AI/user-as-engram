...
\emph{shared} and the facts are \emph{incompressible}: each one must
be stored explicitly and ends up entangled with every other, a
distinct factual share of the weights that does not shrink with
...
fact is written into it (Appendix Figure~\ref{fig:user-insertion}) and
how the writes are served per user (Appendix Figure~\ref{fig:arch}).
...
\textbf{(d)~Where does it break?}
(Sections~\ref{sec:multihop}--\ref{sec:paraphrase}) multi-hop chaining
and paraphrases. The capacity, token-budget, and dense-size
...
(Appendix Figure~\ref{fig:density}).
...
per-user storage (Appendix Figure~\ref{fig:density}; per-$n$ recall and
...
nearest-neighbour retrieval is near-perfect and beats Engram's 68\%
top-1---but it pays 16--63 context tokens for the answer, where Engram
pays none (Appendix Figure~\ref{fig:memsystems}).
...
(Appendix Figure~\ref{fig:memsystems}), because the contract is
...
Appendix Figure~\ref{fig:memsystems}: multi-trigger insertion
...
(Appendix Figure~\ref{fig:layered-arch}).
...
Appendix Figure~\ref{fig:serving-scale} reports throughput, latency,
...
(Appendix Figure~\ref{fig:serving-scale}); each extra user costs one small
