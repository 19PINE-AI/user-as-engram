...
reasoning relative to the untouched base. We can also open the edit
up and watch it work on the trained model: writing a fact switches
on the memory lookup at exactly its trigger, adds the value the
answer needs and nothing else---it leaves every other position
unchanged down to the last bit---and stops working if we write it
into the wrong layer. Each step is measured, not assumed. Against
retrieval, which method wins is decided by how it is deployed, not by
how big the benchmark is: a per-user Engram table does not grow with
the number of users, so past $\sim$100 facts it overtakes a retrieval
pipeline running on a $2.5\times$ larger model. The thread tying this
together is simple: every method that reaches near-perfect recall
pays for it in weaker reasoning, and they differ only in \emph{where}
that price is paid---in the shared weights (LoRA), at one address
(Engram), or in a search index (RAG). We release four Mini-Engram
...
This is not a side-effect of how we trained it. It is the price of
keeping a fact inside a function the whole model shares. A LoRA has
no address: to make one fact more likely, gradient descent has to
bend $Q/K/V/O$ in whatever directions fit it most cheaply, and those
same directions are also in use on unrelated text---so every forward
pass feels the change.

This shows up as two separate costs. The first is extra loss on
unrelated text: it is large, and we see it on every base we tried.
The second is what the user notices---worse reasoning---and it
depends on the base. A heavily instruction-tuned model shrugs the
change off; a weaker base LM does not. Earlier work looked only at
the second cost on strong bases and concluded the recipe was
harmless. The first cost shows the edit was never local
(Section~\ref{sec:negative}).

\paragraph{The principle: put each fact where storage belongs.}
We put the fact where storage belongs---a memory table, not the
shared weights.
...
\paragraph{The same price, paid in different places.}
The three families are not really competing on quality. Each one
reaches near-perfect direct recall, and each one pays for it with
weaker reasoning over the stored facts. What differs is \emph{where}
the price shows up, and how fast it grows as you add facts and users.
A per-user LoRA pays in the shared weights, so the cost is there on
every query and adds up as users are stacked together. An Engram row
pays at the one place its fact is stored, so the cost grows only with
how many facts a single user has. Retrieval pays in the search step,
where the cost grows with the size of the pool the encoder must
search. Asking \emph{where the price is paid} is what turns ``which
method is better'' into a question about how the system will be
deployed---the view the rest of the paper makes concrete.

\paragraph{Contributions.}
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=2pt]
  \item \textbf{The cost of storing facts in shared weights---and
        why earlier work missed it} (Section~\ref{sec:negative}).
        A per-user LoRA has no
        ...
        text---a property of how a LoRA stores a fact, not of how we
        tuned it. This extra loss is always there; what a user
        actually notices---worse reasoning---only shows up on a weak
        base. A strong, instruction-tuned base answers the question
        on its own and hides the damage, which is exactly why earlier
        work judged the recipe harmless. We show you can turn the
        visible damage on and off just by changing how strong the
        base is.
  \item \textbf{User as Engram, with the mechanism opened up}
        ...
        the wrong layer (Figures~\ref{fig:locality}--\ref{fig:glassbox}).
...
the adapter's inability to provide it.

\paragraph{Why earlier work thought this was harmless.} This is
worth saying plainly, because it changes how the whole per-user-adapter
line should be read. \textbf{If you test a per-user LoRA on a strong,
instruction-tuned model, you see neither problem: not the extra loss on
unrelated text, and not the fact that the adapter never learned to
reason over what it stored.} The extra loss is always there, on every
base. The thing a user would actually notice---weaker
reasoning---disappears exactly when the base is strong enough to answer
the question by itself. So the recipe looks fine precisely on the
models people usually test it on. We can make the damage appear or
vanish just by changing how strong the base is
...
Figures~\ref{fig:locality}--\ref{fig:glassbox} show each.
...
\begin{figure}[t]
  ...
  colour scale). \textbf{(a)} An Engram row insertion is
...
in a single user's table, at a fixed inference-time optimisation
...
\textbf{Storage.} A user's whole table is a fixed $\sim$88\,KB, while
a per-user LoRA is 14.2\,MB---$\sim$161$\times$ larger at 100
...
rebuild it (Appendix~\ref{app:locomo}, Figure~\ref{fig:locomo-categories}).
So User-as-Engram is a specialised tool, not a drop-in replacement
...
with an LLM judge (Qwen2.5-14B, Appendix~\ref{app:locomo}), first-token
...
The headline is the F-vs-B contrast (Figure~\ref{fig:layered-conditions}).
The layered design matches per-user LoRA on direct recall
(100\% vs.\ 99\%) but answers indirect questions
\textbf{$7.4\times$ more often} (44\% vs.\ 6\%), with
\textbf{$4.6\times$ less damage to unrelated text} and \textbf{not a
...
The gap is stable across three seeds (paired-bootstrap 95\% CI
$[+31,+37]$\,pp; Appendix~\ref{app:layered-seeds}).
...
The obvious objection to F is: why not just retrieve the facts and put
them in the prompt? We test this two ways (conditions G--J in
Figure~\ref{fig:layered-conditions}; context-cost plot in
Appendix~\ref{app:qwen-rag-prompt}, Figure~\ref{fig:pareto-rag}). On
the Engram base itself, putting facts in the prompt sits \emph{below}
F---the base LM never learned to use an in-context fact list, and even
...
(Qwen2.5-3B-Instruct, about $2.5\times$ larger). Even there, F---with
no retrieval and an empty prompt---lands within a few points of the
larger model, and adding retrieval to our shared skill (J) passes it
once context is free. The plain rule: no room in the prompt $\to$ use
F; room to spare $\to$ add retrieval on top of the shared skill.
...
rewrite. Writing and restoring the rows cost about 2.3\,ms each, and
the forward pass dominates the rest (full breakdown in
Appendix~\ref{app:cdf}).
...
users with the same favourite spice---and it is small.) By contrast,
...
optimisation steps, not by raising the ceiling
(Appendix~\ref{sec:mf-finetune}, Figure~\ref{fig:mf-finetune}). This
pins the density ceiling on inference-time optimisation, not
...
Mini-Engrams (d12@1280 and d20, 16 canonical facts; probes in
\texttt{scripts/mech\_glassbox.py}, \texttt{mech\_lora\_vs\_engram.py},
\texttt{mech\_depth.py}). All numbers below are on the trained
models unless stated.
...
injection but do not redirect it. This reproduces the random-init
read/write probe (cosine $0.998$, Appendix~\ref{app:tier1}) on the
trained model, for the strategy we deploy. (The small-magnitude
...
this from the behavioural side via the sensitivity asymmetry
(Section~\ref{sec:repro}): ablating the Engram pathway
(\texttt{engram\_suppress}) degrades \emph{factual} top-5 recall
...
\emph{optimises}.} A subtler reading concerns the row's relation to
...
reads cleanly, optimised against the true objective.
...
(\texttt{mech\_glassbox.py}): the maximum residual change over every
...
budget, \texttt{mech\_depth.py}): top-1 recall collapses from
...
\begin{figure}[t]
  ...
  RAG reaches 55--57\% at 82--390 tokens on
...
\subsection{Additional figures (schematics and supporting results)}
\label{app:morefigs}
The figures here are referenced from the body; they are placed in the
appendix to keep the body figure-light.
