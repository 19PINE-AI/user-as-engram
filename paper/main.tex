...
There is \emph{content}---the specific facts about a user---and
\emph{meta-skill}---the reasoning that turns those facts into
answers. The human brain keeps the two apart: it writes a new
episode as a sparse, local trace (an \emph{engram}) in the
hippocampus, while the slow neocortex accumulates the shared
skills that interpret it---a separation that lets us learn a new
fact without overwriting everything else we know. Today's
dominant recipe for LLM personalization, the per-user LoRA
adapter, does the opposite: it folds content and skill into one
global weight delta. We show this carries an architectural
price. On a controlled base (Mini-Engram-d20), a per-user LoRA
raises validation loss on text \emph{unrelated} to the user by
$+1.78$ bits/byte, whereas writing the same facts as local
\emph{Engram rows} raises it by $+0.00005$---about
$34{,}000\times$ less, by construction.

We therefore propose \textbf{User as Engram}: store a user's
content as surgical per-user edits to the hash-keyed memory
table of an Engram model, and carry the reasoning skill in one
\emph{shared} adapter amortized across the population---the
artificial analogue of the brain's complementary learning
systems. This layered design matches per-user LoRA's perfect
direct recall while delivering $7.4\times$ higher
indirect-reasoning accuracy and $4.6\times$ less contamination,
and never hurts reasoning relative to the untouched base.
Against retrieval, which substrate wins is decided by deployment
scale rather than benchmark size: a per-user Engram table does
not grow with the user population, so once a user's knowledge
base passes $\sim$100 facts it overtakes a retrieval pipeline
built on a $2.5\times$ larger model. We release four Mini-Engram
checkpoints (178\,M--1.22\,B parameters; the first public
Engrams outside DeepSeek-AI), all code and result data, and a
50-line multi-tenant server that reaches $232$\,req/s with zero
cross-user leakage by construction. User as Engram is the
parametric dual of externalized ``user-as-code''
memory~\citep{li2026userascode}: one compiles the user into
executable code, the other internalizes the user as a local
memory trace.
\end{abstract}
...
and \emph{meta-skill} (the reasoning that turns facts into
answers).
...
on a frozen base (we call this canonical recipe
\textbf{POLAR-class} and use it as our reference per-user-LoRA
baseline: rank-64 on $Q/K/V/O$ in every layer, 12 epochs
$\times$ 200 NTP samples per user). A LoRA is a single global
weight delta---it stores content and skill in the same
parameters and, like nudging every synapse, perturbs the model
everywhere, not only where the user's facts are relevant.
...
user's fact is relevant and is mathematically inert everywhere
else---the computational property the hippocampus buys with
sparse coding. We propose \textbf{User as Engram}: store each of
...
This content/meta-skill split is the artificial counterpart of
complementary learning systems~\citep{mcclelland1995cls}
(Section~\ref{sec:engram-bio}). Locality is a measurable
invariant, not an asymptotic approximation: on Mini-Engram-d12,
a UNEMBED\_P insertion at the last Engram layer perturbs the
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=2pt]
  ...
  \item \textbf{User as Engram: a local-edit substrate}
        ...
        closed-form; OPT independent; Joint OPT, abbreviated
        J-OPT). Joint OPT
...
contradictions and aggregates exactly. The two paradigms are
duals---internalised parametric edits versus externalised
executable state---and we contrast them in
...
\begin{figure}[t]
  ...
  less, by construction. The 3-seed mean ratio is
...
\begin{table}[t]
  ...
  \begin{tabular}{ll}
    ...
    Neocortex: slow, distributed general skills & Shared LoRA + frozen backbone: amortised meta-skill \\
    ...
    Systems consolidation: hippocampus $\to$ cortex & Amortising recurring content into the shared skill \\
    \bottomrule
  \end{tabular}}
\end{table}
...
trigger templates) is plotted in Figure~\ref{fig:density}.
% [reorg] tab:joint-opt-density dropped; curve shown in Figure~\ref{fig:density}.
...
51.2\,M Engram params), top-5 improves slightly at $n{=}100$
(0.96 $\to$ 0.99), but at $n{=}1000$ top-1 \emph{degrades} from
0.35 (d12@768) to 0.28 (d12@1280 and d20@1536). When the table saturates at 1000 facts, gradient
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figs/fig_memsystems.pdf}
  \caption{Direct recall vs.\ memory systems, all sharing the same
  Mini-Engram-d12 answering LM (100 USER facts, XXL corpus).
  \textbf{Left:} when the query is the fact's stored prefix,
  nearest-neighbour retrieval is near-perfect and beats Engram's
  68\% top-1---but at a context-token cost Engram does not pay
  (0 vs.\ 16--63 tokens). \textbf{Right:} when the query is a
  paraphrase, retrieval drops to 60--75\% as the encoder confuses
  surface forms, while multi-trigger Engram insertion (writing at
  every anticipated paraphrase, $\sim$5\,s/fact) reaches 96.9\%
  top-1---a 22-point lead at zero context. The verdict is
  scale-invariant across d12@1280 and d20@1536.}
...
LOCOMO setup (Figure~\ref{fig:locomo-judge}):
...
Pareto choice of Figure~\ref{fig:pareto-rag}.
