...
the trigger N-gram's hash address. The edit is \emph{local}:
on Mini-Engram-d12 we measure exactly 0.000 residual-stream
perturbation at every non-trigger position, through every
subsequent attention+MLP layer (full numbers in
Section~\ref{sec:locality}). We present
(i) a nano-scale public reproduction of Engram at four dense sizes
spanning 178\,M to 1.22\,B total parameters (127\,M to 1.17\,B
non-Engram; 51.2\,M Engram-table params)—the first publicly
available Engram models outside DeepSeek-AI;
...
100 facts/user on Mini-Engram-d12 v2 vs.\ 36\% / 54\% for
independent OPT);
...
d20@1536 (token-F1 +57\%, LLM-judge accuracy +18\% over the best
retrieval baseline; Table~\ref{tab:locomo-multitoken}). The first-token
OPT objective wins on token-F1 but loses under LLM-judge by
$-$0.05; \textbf{multi-token answer-conditioned OPT closes the gap
and flips the ranking back to Engram winning}. Engram further beats
...
scale (measured at 30 users $\times$ 50 facts; the 100-user
$\times$ 100-fact configuration is exercised for correctness but
throughput is not benchmarked here), and zero cross-user leak by
construction;
...
User-as-Engram edits are \emph{local}: they don't pay the reasoning
tax LoRA-as-memory does (we measure exactly 0.000 contamination of
non-trigger forwards through 5 subsequent attention+MLP layers), and they win on
the LOCOMO multi-hop and reasoning categories (+178\% and +58\%
token-F1 over the best retrieval baseline). We caution that the
multi-hop win is partial: Section~\ref{sec:multihop} shows it
accrues to queries whose surface form happens to share a suffix
N-gram with the second-hop trigger; chained queries with no surface
overlap remain the open problem we share with LoRA-as-memory. With Joint OPT, per-user override
...
\texttt{large} (50\,K $\times$ 256, 51.2\,M params) across both
probed dense sizes; per-fact independent OPT recall is flat up to
1\,000 facts.

The empirical headline depends on training objective. The
first-token OPT objective wins under token-F1 (the gold first
token reliably appears) but loses under LLM-judge by 0.05--0.10
because the remaining 15-token continuation is unguided.
\textbf{Multi-token answer-conditioned OPT flips the ranking back
to Engram winning on LOCOMO single-hop under both metrics
simultaneously} (d20@1536: token-F1 0.310 vs.\ 0.197 best
retrieval, +57\%; LLM-judge 0.225 vs.\ 0.190 best retrieval,
+18\%; Table~\ref{tab:locomo-multitoken}).
The category breakdown is more nuanced: Engram wins multi-hop
(token-F1 +178\%), reasoning (+58\%), and single-hop with
multi-token OPT, but loses open-domain ($-$53\%) where the answer
is verbatim in a single retrieved evidence sentence.

The architectural result is independent of the metric debate.
Surgical row insertion, additive composition, per-user override
tables, and multi-tenant serving together unlock zero-leak personal
memory at storage costs orders of magnitude below per-user LoRA.
The recommendation is twofold: (i) match the surgical training
objective to the deployment metric (multi-token where free-form
answers matter); (ii) treat substrate selection—not gradient
training—as the bottleneck of personal memory. The right substrate
is hash-keyed embedding tables.
