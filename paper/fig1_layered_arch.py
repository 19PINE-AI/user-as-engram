...
C_LORA     = "#aa4499"  # purple: per-user LoRA (the bad baseline)
C_BASE     = "#888888"
C_TEXT     = "#222222"
...
gs = fig.add_gridspec(1, 4, width_ratios=[1.05, 1.05, 1.05, 1.35],
                      wspace=0.45, left=0.03, right=0.98, top=0.92, bottom=0.10)
...
ax.text(5.0, 0.7,
         r"$\Delta$bpb on unrelated text $=\;+0.0001$",
         fontsize=8.5, ha="center", color=C_CONTENT, fontweight="bold")
ax.text(5.0, 0.0,
         "(fires only on trigger N-gram)",
...
ax.set_xlim(0, 10); ax.set_ylim(0, 10); ax.axis("off")
ax.set_title("(b)  Shared meta-skill\n(one LoRA, 12\,MB, amortised)",
              fontsize=9.5, color=C_TEXT)

# Large central box = the shared LoRA
...
ax.text(5.0, 4.8, r"\textit{``Facts: $\cdots$ \;\;Q: $\cdots$\;\; A: $\cdots$''}", fontsize=8, ha="center", color="#444")

# Two converging arrows from "training users" to LoRA
for x0 in (2.0, 8.0):
...
ax.text(5.0, 1.0,
         r"$\Delta$bpb on unrelated text $=\;+0.39$",
         fontsize=8.5, ha="center", color=C_SKILL, fontweight="bold")
ax.text(5.0, 0.3,
         "(amortised across 1 M+ users)",
         fontsize=7.5, ha="center", color="#555", style="italic")
...
# Per-user Engram override (top-half illustrative)
...
# Shared LoRA delta on Q/K/V (bottom-half illustrative)
...
ax.annotate("forward", xy=(8.6, 4.7), xytext=(8.6, 4.7),
              fontsize=7, color="#444")
ax.annotate("", xy=(8.7, 8.4), xytext=(8.7, 1.4),
              arrowprops=dict(arrowstyle="<->", color="#888", lw=1.0))

ax.text(5.0, 0.3,
         "total per-user storage: 88\,KB",
         fontsize=8.5, ha="center", color=C_TEXT, fontweight="bold")

# -----------------------------------------------------------------------------
# Panel (d): the six-condition Pareto
# -----------------------------------------------------------------------------
ax = fig.add_subplot(gs[0, 3])
# Re-implement the Pareto with cleaner styling and explicit labels
# (storage_KB_per_user, indirect_any %, label, color, marker, Δbpb)
pts = [
    (0.5,   0.192, "A: no edit",                       C_BASE,    "x",  0.000),
    (14200, 0.072, "B: per-user LoRA",                 C_LORA,    "*",  1.78),
    (88,    0.233, "C: per-user Engram",               C_CONTENT, "o",  0.000),
    (14288, 0.080, "D: B + C\n(naive composition)",    "#cc6677", "v",  1.42),
    (0.5,   0.440, "E: shared LoRA only",              "#88ccee", "D",  0.39),
    (88,    0.443, r"\textbf{F: LAYERED}"+"\n(shared LoRA + Engram)",
                                                       C_SKILL,   "P",  0.39),
]
for x, y, label, color, marker, _ in pts:
    sz = 220 if marker == "P" else 140
    ax.scatter([x], [y], color=color, marker=marker, s=sz,
                 edgecolor="black", lw=0.7, zorder=4)

# Hand-placed text labels
ax.annotate("A: no edit",       (0.5, 0.192), xytext=(1.5, 0.155), fontsize=8, color=C_BASE)
ax.annotate("B: per-user LoRA", (14200, 0.072), xytext=(1300, 0.058), fontsize=8, color=C_LORA)
ax.annotate("C: per-user Engram", (88, 0.233), xytext=(165, 0.218), fontsize=8, color=C_CONTENT)
ax.annotate("D: B + C (naive)", (14288, 0.080), xytext=(1300, 0.108), fontsize=8, color="#cc6677")
ax.annotate("E: shared LoRA only", (0.5, 0.440), xytext=(1.5, 0.418), fontsize=8, color="#5588aa")
ax.annotate(r"\textbf{F: LAYERED}", (88, 0.443), xytext=(160, 0.452), fontsize=9.5, color=C_SKILL, fontweight="bold")
ax.annotate("(shared LoRA + Engram)", (88, 0.443), xytext=(160, 0.432), fontsize=7.8, color=C_SKILL)

# Δbpb annotations next to the contaminating points
ax.text(14200, 0.020, r"$\Delta$bpb = $+1.78$", fontsize=7.5, color=C_LORA, ha="center")
ax.text(88,    0.475, r"$\Delta$bpb = $+0.39$", fontsize=7.5, color=C_SKILL, ha="center")

# Pareto frontier connector for F & E
ax.plot([0.5, 88], [0.440, 0.443], "k--", alpha=0.35, lw=0.9, zorder=2)

ax.set_xscale("log")
ax.set_xlim(0.25, 60000)
ax.set_ylim(0.0, 0.52)
ax.set_xlabel("per-user storage (KB, log scale)")
ax.set_ylabel("indirect reasoning  (\\textit{any-match}, $n{=}20$)")
ax.set_title("(d)  Pareto on Mini-Engram-d20\n(same base, six substrate combos)",
              fontsize=9.5, color=C_TEXT)
ax.grid(True, which="both", alpha=0.18, lw=0.4)
ax.spines[["top", "right"]].set_visible(False)

fig.savefig(OUT / "fig1_layered_arch.pdf", bbox_inches="tight")
print(f"wrote {OUT / 'fig1_layered_arch.pdf'}")
plt.close(fig)
