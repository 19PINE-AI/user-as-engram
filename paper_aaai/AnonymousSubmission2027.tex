...
is a higher-bandwidth interface to frequently accessed user state than passing
a few retrieved text snippets through the prompt; it complements rather than
...
intrinsically require training a per-user adapter---closed-form insertion costs
${<}1$\,ms, and deletion removes addressed rows with no gradient steps;
...
addressed deletion, and composable maps. Second, we separate this mutable
...
access or global weight changes. User as Engram occupies a distinct middle
point: records remain explicit and mutable like an external store, but their
values are consumed inside the network without retrieval calls or context
tokens. Our content/skill boundary places those mutable facts in addressed rows
and reusable reasoning in shared weights, yielding locality, high-frequency
memory access, and multi-tenancy rather than dense per-user models.
...
LoRA, and uses $161\times$ less state at 100 facts while enabling training-free
insertion and deletion. Its token-granular lookup removes external memory hops
and remains robust as retrieval recall declines.
