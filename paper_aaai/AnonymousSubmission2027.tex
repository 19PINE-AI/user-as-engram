...
20 users and probes. All conditions use the same Mini-Engram-d20 answering
backbone. Retrieval coverage falls with distractors, whereas addressed Engram
lookup is corpus-size independent
(Figure~\ref{fig:rag-scale}).

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figs/fig_rag_scale.pdf}
  \caption{Knowledge-base scaling on the same Mini-Engram-d20 backbone.
  RAG top-$k$ places the $k$ highest-ranked facts in context; the shared
  reasoning LoRA is the fixed rank-16 cross-user adapter also used by the
  layered model. Because the adapter does not change retrieval, panel (b)
  shows only one top-3 recall curve.}
  \label{fig:rag-scale}
\end{figure}
