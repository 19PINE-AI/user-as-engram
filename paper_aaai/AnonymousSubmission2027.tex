...
% Reduce the gap between top-of-page figures and the following body text.
...
Engram}: encode each user fact as embedding values and write them directly into
deterministically addressed rows of a pretrained Engram memory table. These
surgical weight edits carry the user knowledge; changing them does not retrain
the backbone or its shared reasoning capability. Against per-user LoRA, the
design provides three benefits.
At 100 facts it uses 88\,KB rather than 14.2\,MB ($161\times$ less state);
closed-form writes and addressed deletion avoid per-user retraining; and it
...
Personalization requires a model to acquire mutable user facts without turning
every recall into an external retrieval or every update into global model
adaptation. Retrieval-augmented generation (RAG) keeps facts editable, but each
recall searches a store, serializes retrieved text, and pays the resulting
context and prefill cost \citep{lewis2020rag,gao2023ragsurvey}; an agentic
implementation also generates and executes a tool call. Per-user LoRA
\citep{hu2022lora} removes that external path, but represents a user's records
as a model-wide low-rank update. It requires per-user optimization, provides no
explicit address for replacing or deleting one fact, consumes 14.2\,MB at rank
64 even for a small record set, and can perturb unrelated behavior. Neither
approach provides an internal fact store whose individual writes are explicit
and structurally local.

We propose \emph{User as Engram}: write facts directly into the model as
surgical edits to an Engram memory table \citep{cheng2026engram}. For each
fact, deterministic hashes of its trigger suffixes select a small set of memory
rows; we encode the fact as value embeddings and override only those addressed
parameters. The rows are part of the pretrained model, and their gated values
enter the residual stream during the same forward pass as ordinary Transformer
computation. Every changed address is therefore enumerable, while the backbone
and all unaddressed rows remain bit-identical. Unlike LoRA, the fact is not
distributed through a model-wide update; unlike RAG, it is not fetched and
serialized as text at query time.

The Engram embeddings---not a per-user adapter---carry the user-specific
knowledge. Engram pretraining establishes the address-and-gate pathway that
indexes these rows, while the model's pretrained or post-trained general
capability determines how their values are used. In our experiments, one
shared LoRA strengthens this general reasoning capability, but it is only a
complementary reader: it contains no test-user facts and remains fixed when an
Engram fact is inserted, replaced, or deleted. Changing user knowledge thus
changes only the addressed Engram rows and never requires retraining the
backbone or reasoning skill.

This representation makes internal memory directly manageable. A 100-fact
override map is 88\,KB, $161\times$ smaller than a rank-64 per-user LoRA;
closed-form insertion costs ${<}1$\,ms, addressed deletion requires no gradient
steps, and optional row-only refinement can repair interactions among facts.
At inference, suffix $N$-grams can activate these embeddings at every prefill
or decode position without semantic search, a tool round trip, or retrieved
context tokens. Across three seeds, Engram facts read by the shared capability
reach 41.2\% indirect accuracy versus 7.3\% for per-user LoRA, and their largest
LOCOMO gains occur on multi-hop questions. Figure~\ref{fig:benefits} summarizes
the storage, lifecycle, and use benefits.

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.80\textwidth,trim=0 34bp 0 0,clip]{figs/fig_layered_arch.pdf}
  \captionsetup{skip=3pt}
  \caption{User facts reside in surgical Engram-row weight edits. A shared
  pretrained or post-trained capability reads them and remains fixed when the
  facts are inserted, replaced, or deleted.}
  \label{fig:layered-arch}
\end{figure*}

Figure~\ref{fig:layered-arch} illustrates the roles. A user's doctor is stored
in the highlighted Engram embeddings; if that doctor changes, only those rows
are replaced. Inferring that a cardiologist is relevant to a heart check-up is
a general capability that may be learned during pretraining or post-training
and reused unchanged. The reader enables use of the memory, but the fact itself
resides entirely in the surgical Engram edit.

\paragraph{Contributions.}
We make three contributions. First, we introduce a protocol that writes a new
user fact as embedding-valued, deterministically addressed edits to an
Engram-pretrained model's memory parameters. Second, we develop closed-form,
per-fact, and joint writes together with addressed deletion, composable
per-user maps, and mechanistic audits of the exact edit footprint. Third, we
compare this internal memory with per-user LoRA and RAG across storage, update
cost, locality, reasoning, context tokens, and knowledge-base scaling, and test
whether a user-independent capability can operate on facts stored only in
Engram rows. Engram uses $161\times$ less per-user state than LoRA and no
external memory hop, while the $2.5\times$ larger Qwen-RAG baseline falls below
it beyond 100 facts (Figures~\ref{fig:benefits}, \ref{fig:pareto-rag}, and
\ref{fig:rag-scale}).
...
do not alter its addressing rule or backbone. Our primary contribution is the
write and composition protocol that turns selected Engram parameters into an
editable store for new user facts. The shared adapter is not part of that
store; it is a controlled way to test whether a user-independent reasoning
capability can consume the writes, and it remains unchanged when facts change.
...
the row contribution with the answer token's unembedding direction; it costs
${<}1$\,ms and requires no gradient steps. Per-fact gradient optimization
...
is nonzero at every position and layer and moves unrelated text by a mean
residual norm of 107. Locality therefore follows from the data structure
...
before optimization. Deleting a user or fact removes its override entries
rather than reversing a gradient update; updating a fact replaces explicit
entries. Closed-form writes need no training, while optional joint
re-optimization can repair interference when facts share rows.
...
No pretrained Mini-Engram checkpoints are publicly available. We therefore
...
bf16 and the nanochat budget of 12 tokens per scaling parameter. Base, XL, and
...
\texttt{indirect\_any} finds the gold span anywhere, and LOCOMO uses token-F1.
Contamination is the change in held-out bits per byte
($\Delta$bpb) on unrelated ClimbMix: 262K tokens for the canonical run and
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figs/fig_user_memory_benefits.pdf}
  \caption{Engram-row benefits over per-user LoRA: \textbf{(a)} storage,
  \textbf{(b)} lifecycle, and \textbf{(c)} downstream use.}
  \label{fig:benefits}
\end{figure}
...
density, with top-5 recall above 90\% to roughly 300 facts/user. At 100 facts,
its sparse override occupies 88\,KB versus 14.2\,MB for rank-64 LoRA. Unlike a
model-shaped adapter, each row remains an explicit item that can be replaced or
removed; UNEMBED\_P provides a no-training write path, while OPT and Joint OPT
trade write compute for recall. Figure~\ref{fig:benefits} summarizes these
storage, lifecycle, and downstream-use distinctions.
...
Taken together with Figure~\ref{fig:benefits}'s controlled LoRA comparison,
these results show that Engram facts can serve as operands for held-out
transformations and multi-hop questions. Raw rows do not supply the reasoning
algorithm: the key is that one shared skill can consume local facts, rather
than relearning both content and skill inside every user's adapter.
...
while fitting only test-user rows. The resulting layered system reaches 41.2\%
indirect accuracy across seeds, $5.6\times$ the per-user-LoRA baseline; the
LOCOMO category results above independently show the largest gains on
multi-hop questions.
...
\section{Reasoning over Engram-Stored Facts}
\label{sec:layered}
...
We use a rank-16 shared LoRA as a controlled reader of Engram-stored facts, not
as the user-memory store. We train it on 10 users with each indirect question's
needed facts in context, freeze it, then test 20 disjoint users whose facts
arrive only through Engram rows. Replacing a test-user fact changes only that
user's row map. Six conditions cross content placement (none, per-user LoRA, or
Engram) with shared skill while holding direct recall nearly constant. Across
three seeds, layered Engram reaches 100\% versus 98.8\% direct recall and 41.2\%
versus 7.3\% indirect accuracy for per-user LoRA.
...
\begin{figure}[t]
  ...
  \caption{Engram versus RAG: indirect accuracy against extra serialized
  memory tokens per query; RAG points each perform one search.}
...
The distinction is access granularity. RAG searches once and passes a small
set of facts through the prompt; an agentic pipeline additionally generates
and executes the tool call, and sequential recalls repeat that external hop.
Engram instead computes suffix addresses at every prefill and decode position
and injects selected values inside the forward pass. It therefore supplies a
token-granular memory channel without retrieved context, while retaining RAG
as the better complement for rapidly changing corpora, provenance, and
open-domain evidence.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figs/fig_rag_scale.pdf}
  \caption{Engram versus RAG as the knowledge base grows: \textbf{(a)}
  indirect accuracy and \textbf{(b)} shared retriever recall. Qwen-3B uses a
...
20 users and probes. The Mini-Engram RAG conditions use the same answering
backbone as Engram; Qwen-RAG uses the larger instruction-tuned model. RAG
top-$k$ places the $k$ highest-ranked facts in context, and all RAG conditions
share the same retriever. Its top-3 recall falls from 62.0\% at 34 facts to
9.3\% at 1,000. Qwen-RAG top-3 consequently falls from 51.8\% to 30.0\%: it
ties Engram's constant 44.5\% at 100 facts and trails thereafter. Same-base RAG
top-3 falls from 38.8\% to 29.8\%. RAG plus the shared adapter remains slightly
ahead at 1,000 facts (47.0\%), again favoring a hybrid when retrieval's
provenance is needed (Figure~\ref{fig:rag-scale}).

\section{Multi-Tenant Serving}
\label{sec:serving}

A request resolves a sparse user table, saves and overrides affected base rows,
runs one forward pass, then restores them. During that pass, each token's
suffix can fetch a gated memory value directly into the residual stream; there
is no separate retrieval service or memory text to prefill. User switches
change neither dense weights nor the shared adapter. Disjoint personal, team,
and organizational scopes can layer; overlapping triggers are
last-write-wins and should remain separate. Unmeasured batched serving would
require one gather per Engram layer.
...
On one idle Blackwell GPU, d12@1280 processed 232 requests/s with 30 users and
50 facts/user (4.2\,ms p50 latency), and 226 requests/s with 100 users and 100
facts/user (4.4\,ms p50). These measurements are consistent with per-request
work depending on the active override rather than the registered-user
population, but they are not a controlled user-count scaling study because
the fact count also changes. Absolute latency depends on cache residency,
hardware, and the serving implementation.

\section{Discussion, Limitations, and Related Work}
\label{sec:discussion}

\paragraph{Limitations.}
Within-user capacity remains limited: at 1,000 facts Joint OPT reaches 35\%
top-1 despite ${<}1\%$ occupancy, indicating forward-pass interference rather
than collisions. Recall depends on trigger and paraphrase coverage, and
insertion is slower than appending a document. LOCOMO mixes memory with base
knowledge; storage and latency are implementation-dependent. We have not tested
batched serving, adversarial exfiltration, or production access controls. Most
controlled tests use templated, short-answer facts; LOCOMO adds conversational
evidence, but does not establish long-form generation, temporally conflicting
memories, or semantic access without a matching trigger.
...
\citep{lample2019pkm,packer2023memgpt,cheng2026engram}. Unlike external access
or global weight changes, User as Engram stores explicit, mutable records as
surgical edits to addressed model parameters and consumes them in-network.
This supports local, high-frequency, multi-tenant memory. The measured
dense-adapter interference relates to catastrophic forgetting
\citep{mccloskey1989catastrophic,kirkpatrick2017ewc}.
Compared with conventional model editing, the inserted fact need not already
exist in dense weights, and locality follows from address-conditioned activation
rather than a learned regularizer. Compared with per-user adapters, storage
grows with the written rows rather than model dimensions. These structural
advantages make updates inspectable and reversible, but constrain recall to the
trigger contract.

\paragraph{Societal considerations.}
Addressability aids inspection and erasure but can expose records; output
auditing and shared-skill fairness also remain open concerns.

\paragraph{Future work.}
Future recipes should test multi-fact training, random per-user hash offsets,
and ``recall, then reason'' examples \citep{sun2023recitation}. Preliminary
multi-fact finetuning sped convergence without raising the ceiling; teacher
traces lowered accuracy when training and evaluation formats diverged.

Because our current pretraining reaches only 1.22B parameters, future work
should scale the architecture and training recipe to substantially larger
models. This will test whether its locality, recall, and multi-user efficiency
persist at frontier-relevant scales.

\section{Conclusion}

User as Engram writes user facts directly into a model as embedding-valued,
surgical edits to deterministically addressed Engram rows. Only those
parameters change, and their gated values are read in the ordinary forward
pass. At 100 facts, it matches per-user LoRA recall with $161\times$ less state
and about $33{,}000\times$ less held-out loss increase, while requiring neither
RAG's retrieval call nor serialized memory text. A shared reader improves
indirect reasoning by $5.6\times$ over per-user LoRA but contains no user facts:
knowledge remains in the Engram edits, so facts can be inserted, replaced, or
deleted without retraining that capability. This gives model-internal personal
memory the lifecycle of an editable store.
